...
class EncryptedEchoDVM:
    ...
    def __init__(
        self, keys: Keys, relay_urls: List[str], dvm_name: str = "Encrypted Echo DVM"
    ):
        self.keys = keys
    ...
    async def subscribe_to_encrypted_requests(self):
        ...
        encrypted_filter = (
            Filter().kind(Kind(1059)).since(Timestamp.now())  # Gift wrap events
        )
    ...
    async def decrypt_gift_wrap(self, gift_wrap_event: Event) -> Optional[Event]:
        """
        Decrypt a gift wrap event using the SDK's proper NIP-59 handling.

        Returns the job event (kind 25000) directly, or None if decryption fails.
        """
        try:
            # Use the SDK's proper gift wrap decryption
            signer = NostrSigner.keys(self.keys)
            unwrapped_gift = await UnwrappedGift.from_gift_wrap(signer, gift_wrap_event)

            sender = unwrapped_gift.sender()
            rumor = unwrapped_gift.rumor()  # This is an UnsignedEvent

            print(
                f"✅ Successfully decrypted gift wrap from {sender.to_bech32()[:16]}..."
            )

            # The rumor (UnsignedEvent) should be kind 1060 containing the job event as JSON
            if rumor.kind().as_u16() != 1060:
                print(f"⚠️ Expected rumor (kind 1060), got kind {rumor.kind().as_u16()}")
                return None

            # The rumor's content contains the job event as JSON
            job_event_json = rumor.content()

            # Parse the job event from JSON - it's unsigned since it was created with .build() not .sign()
            try:
                job_event = UnsignedEvent.from_json(job_event_json)
            except:
                # If unsigned parsing fails, try as signed event
                print(
                    f"Parsing as unsigned event failed, trying again as signed event..."
                )
                job_event = Event.from_json(job_event_json)

            # Verify it's a job request (kind 25000)
            if job_event.kind().as_u16() != 25000:
                print(
                    f"⚠️ Expected job request (kind 25000), got kind {job_event.kind().as_u16()}"
                )
                return None

            print(f"📋 Extracted job request (kind {job_event.kind().as_u16()})")
            return job_event
    ...
    async def send_encrypted_result(
        ...
        try:
            ...
            gift_wrap_builder = EventBuilder(
                kind=Kind(1059), content=encrypted_payload  # The encrypted rumor
            )
            ...
            print(
                f"🔐 Sent encrypted {status_msg} result for job {original_event.id().to_hex()[:8]}"
            )
    ...
    async def handle_encrypted_request(self, gift_wrap_event: Event):
        ...
        1. Decrypt the gift wrap and extract the job event
        2. Process the job
        3. Send back an encrypted response
        """

        print(f"\n=== 🔐 Processing Encrypted Request ===")
        print(f"Gift wrap ID: {gift_wrap_event.id().to_hex()[:8]}...")

        # Step 1: Decrypt the gift wrap and get the job event directly
        job_event = await self.decrypt_gift_wrap(gift_wrap_event)
        if not job_event:
            print("Failed to decrypt gift wrap and extract job event - skipping")
            return
        ...
        class NotificationHandler(HandleNotification):
            ...
            async def handle(self, relay_url: str, subscription_id: str, event: Event):
                ...
                if event.kind().as_u16() == 1059:
                    print(
                        f"🔍 DEBUG: Received gift wrap {event.id().to_hex()[:8]} from {event.author().to_bech32()[:16]}..."
                    )
                    ...
                    print(
                        f"   Gift wrap not for us (p-tags: {[tag.as_vec() for tag in event.tags().to_vec() if tag.as_vec()[0] == 'p']})"
                    )
